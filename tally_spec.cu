...
__global__ void tally_spec_kernel(unsigned N, unsigned Ntally, unsigned tally_cell,  unsigned* remap, source_point* space, float* E, float * tally_score, float * tally_square, unsigned * tally_count, unsigned* done, unsigned* cellnum, unsigned* rxn, float* weight){
...
void tally_spec(unsigned NUM_THREADS,  unsigned N, unsigned Ntally, unsigned tally_cell, unsigned* remap, source_point * space, float* E, float * tally_score, float * tally_square, unsigned * tally_count, unsigned* done, unsigned* cellnum, unsigned* rxn, float* weight){
	
	unsigned blks = ( N + NUM_THREADS - 1 ) / NUM_THREADS;

	cudaDeviceSynchronize();
	tally_spec_kernel <<< blks, NUM_THREADS >>> ( N, Ntally, tally_cell, remap, space, E, tally_score, tally_square, tally_count, done, cellnum, rxn, weight);
	cudaDeviceSynchronize();
