...
__global__ void macroscopic_kernel(unsigned N, unsigned n_materials, unsigned n_tallies, cross_section_data* d_xsdata, particle_data* d_particles, tally_data* d_tally, unsigned* d_remap, float* d_number_density_matrix){
	...
	tope = sample_cross_section(	n_isotopes, macro_t_total, get_rand(&rn),
									e0, e1, this_E,
									&d_number_density_matrix[this_mat],  
									&xs[dex*n_columns],  
									&xs[(dex+1)*n_columns]					);
	...
	if(this_rxn==0){
		...
		if(tope>=n_isotopes){
			...
			col_start	= 	n_isotopes + rxn_numbers_total[tope];
			col_end  	= 	n_isotopes + rxn_numbers_total[tope+1];
		}

	
		// compute the total macroscopic cross section for this isotope (macroscopic for simplicity/function regularity, micro scopic would be fine).  Remeber that total xs is stored in the first n_isotopes of columns, then come the individual reaction cross sections...
		macro_t = = sum_cross_section(	1,
										e0, e1, this_E,
										&d_number_density_matrix[this_mat],  
										&xs[ dex   *n_columns + tope],  
										&xs[(dex+1)*n_columns + tope] );
	
		// determine the reaction for this isotope
		this_rxn = sample_cross_section(	(col_end-col_start), macro_t, get_rand(&rn),
											e0, e1, this_E,
											&d_number_density_matrix[this_mat],  
...
void macroscopic(unsigned NUM_THREADS, unsigned N, unsigned n_materials, unsigned n_tallies, cross_section_data* d_xsdata, particle_data* d_particles, tally_data* d_tally, unsigned* d_remap, float* d_number_density_matrix ){

	unsigned blks = ( N + NUM_THREADS - 1 ) / NUM_THREADS;

	macroscopic_kernel <<< blks, NUM_THREADS >>> ( N, n_materials, n_tallies, d_xsdata, d_particles, d_tally, d_remap, d_number_density_matrix);
